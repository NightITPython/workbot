...
def add_main_chat_handler():
    router = Router()
    @router.callback_query(F.data == "edit_bot:add_main_chat", isTc())
    async def add_main_chat(
        c: CallbackQuery,
        state: FSMContext
    ):
        ...
        await state.set_state(EditBot.add_main_chat)
    
    
    @router.message(F.text, EditBot.add_main_chat)
    async def answer(
        msg: Message,
        state: FSMContext,
        bot: Bot
